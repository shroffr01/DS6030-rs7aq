...
for (i in 2:68){
  ...
  for (m in 1:10) {
    ...
    df_train_mod <- df_train_int
    df_train_mod[[i]] <- sample(df_train_int[[i]])
    
    log_model <- ranger(outcome~.,family='binomial', data=df_train_mod, type='response')
    
    pred <- predict(log_model, newdata = df_test_mod)
