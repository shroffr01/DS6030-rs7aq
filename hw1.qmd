...
The results are not as expected. The lowest MSE on the test data occurs when the degree = 1, not 2. This may suggest that we are dealing with a linear model not a quadratic one.
:::
...
f_function(10000, 2, data_test_new)
```
...
f_function(300, 4, data_test_new)
```
:::
...
::: {.callout-note title="Solution"}
As standard deviation increases, there is more variability in which model is the best. As n increases, there is less variability since there is more data. The true model form (quadratic) is not always the best model to use when prediction is the goal because it might be overfit to the data.
:::
