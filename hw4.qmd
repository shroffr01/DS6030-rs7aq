...
df <- tibble(Pm, Gini, Classification, Entropy)
df <- df %>% mutate_at(1:3, as.numeric)
df[is.na(df)] <- 0

ggplot(df, aes(x=Pm)) + 
  geom_line(aes(y=Gini, color = 'Gini')) + 
  geom_line(aes(y=Classification, color = 'Classification')) +
...
::: {.callout-note title="Solution"}
Looking at the ranger function, the most important parameters are mtry (# of predictors) and min.obs (min obs in leaf). Since we are dealing with a regression tree, mtry = p/3 and min.obs = 5.
:::
...
10 fold CV random forest results in a mean RMSE of 26,459.35 for sale price.
:::
...
::: {.callout-note title="Solution"}
```{r}
mtry_list <- seq(3,25,1)
bucket_list <- seq(1,6,1)

for (i in mtry_list) {
  
  for (b in bucket_list){
    
    rf_1 <- ranger(Sale_Price~., num.trees=1000, data=ames_df)
    rf_2 <- ranger(Sale_Price~., num.trees=1000, data=ames_df)
    rf_3 <- ranger(Sale_Price~., num.trees=1000, data=ames_df)
    rf_4 <- ranger(Sale_Price~., num.trees=1000, data=ames_df)
    rf_5 <- ranger(Sale_Price~., num.trees=1000, data=ames_df)
    
    MSE <- mean(rf_1$prediction.error, rf_1$prediction.error, rf_1$prediction.error, rf_1$prediction.error, rf_1$prediction.error)
...
```{r}
mean1 <- mean(rf_1$prediction.error, rf_1$prediction.error)
mean1
```
:::
