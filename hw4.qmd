...
mtry_vals <- seq(3,15,1)
bucket_vals <- seq(1,6,1)

results_df <- data.frame(bucket=integer(), mtry = integer(), MSE=integer())
...
for (i in mtry_vals) {
  ...
  for (b in bucket_vals){
    ...
    MSE <- mean(rf_1$prediction.error, rf_2$prediction.error, rf_3$prediction.error, rf_4$prediction.error, rf_5$prediction.error)
...
List all parameters for 2a?

How to set seed for 2c?
