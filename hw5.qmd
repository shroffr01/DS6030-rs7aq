...
```{r}
class(predictions_lin)
```

```{r}
roc_lin <- roc(linkage_train[,9], predictions_lin)
roc_log <- roc(linkage_train[,9], predictions_log)
roc_rf <- roc(linkage_train[,9], predictions_rf$predictions)
```

```{r}
roc_lin_data <- data.frame(FPR = roc_lin$specificities, TPR = roc_lin$sensitivities, model = 'Linear model')
roc_log_data <- data.frame(FPR = roc_log$specificities, TPR = roc_log$sensitivities, model = 'Logistic model')
roc_rf_data <- data.frame(FPR = roc_rf$specificities, TPR = roc_rf$sensitivities, model = 'Random Forest model')
