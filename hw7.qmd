...
rf_model <- ranger(SalePrice~., data = df_train_int)
rf_model$prediction.error
```

```{r}
lin_model <- lm(SalePrice~., data=df_train_int)
predicted_values <- predict(lin_model, df_train_int)
...
x_train <- as.matrix(df_train_int[,-38])
y_train <- df_train_mod$SalePrice

lasso_model <- glmnet(x_train, y_train, alpha=0.7, lambda=0.2)
...
rf_pred = predict(rf_model, data = df_test_int) # random forest
lin_pred = predict(lin_model, data = df_test_int) # lin reg

x_test <- as.matrix(df_test_int)
