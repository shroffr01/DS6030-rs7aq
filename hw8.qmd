...
```{r}
# full_sequence <- seq(from = floor_date(min(df$datetime), "hour"),
#                      to = ceiling_date(max(df$datetime), "hour"),
#                      by = "hour")
# 
# missing_hours <- setdiff(full_sequence, df$datetime)
# missing_hours_readable <- as.POSIXct(missing_hours, origin = "1970-01-01", tz = "EST")
# df_time <- data.frame(missing_hours_readable)
# 
# filtered_df_time <- df_time %>% filter(missing_hours_readable != "2011-03-15 03:00:00 EST" & missing_hours_readable != "2011-11-06 01:00:00 EST")
# 
# filtered_df_time
```

```{r}
df_merge <- rbind(df, filtered_df_time)
df_merge <- df_merge[order(df_merge$datetime_col), ]
